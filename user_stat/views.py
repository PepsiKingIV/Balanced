...
def modification_for_day(records) -> list:
    ...
    for el in records:
        ...
        month = int(date[5:7])
        day = int(date[8:10])
        day_number = int(str(datetime.date(datetime(year, month, day)))[5:7])
...
def make_out_data_day(records):
    list_of_records_per_mounth = {"period": [], "amount": []}
    for i in range(32):
...
def make_out_data_week(records):
    ...
    for el in records:
        for i in range(53):
            ...
            day = int(date[8:10])
            week_number = int(
                datetime.date(datetime(year, month, day)).isocalendar()[1]
            )
            if week_number == i:
                list_of_records_per_mounth["amount"][i] += float(round(el.amount))
    return list_of_records_per_mounth
...
class histogram:
    _list_of_records_per_mounth = {"category": [], "amount": []}
    _records = None
    _summ = 0
    _summ_of_enother = 0
    ...
    def __init__(self, records) -> None:
        ...
        self._list_of_records_per_mounth = {"category": [], "amount": []}
        for el in records:
    ...
    def populating_lists(self):
        for i in range(len(self._list_of_records_per_mounth["category"])):
            ...
            if category_amount / self._summ > 0.01:
                self._list_of_records_per_mounth["amount"][i] = round(category_amount)
            else:
